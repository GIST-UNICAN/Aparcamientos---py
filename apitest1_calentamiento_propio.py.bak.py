...
def genera_seccion_aparcamiento(seccion_destino):
    ...
    def calcula_utilidad(fila):
        if fila.seeciones in secciones_parking_subterraneo:
    ...
    df_calculos['calle']=[1 if x in secciones_parking else 0 for x in df_calculos['secciones']]
    ...
    fila_maxima_utilidad = df.iloc[indice_fila_maxima_utilidad]
...
def AAPIInit():
    ...
    dict_plazas_seccion={x:int(y) for x, y
                         in zip(
                             dict_plazas_seccion_list.values()[0],
                             dict_plazas_seccion_list.values()[1])}
    #generamos la lista de comercios por seccion y la p de acabar en cada uno de ellos
    lista_secciones_con_comercios=dict_plazas_seccion_list.values()[0]
    lista_probabilidades=dict_plazas_seccion_list.values()[3]
...
def AAPIEnterVehicle(idveh,idsection):
    ...
    try: 
        ...
        if (info_estatica_vehiculo.centroidDest in centroides_objetivo
            ...
            seccion_aparcamiento, es_parking=genera_seccion_aparcamiento(seccion_destino)
            centroide_aparcamiento=dict_centroide_secciones[seccion_aparcamiento]
